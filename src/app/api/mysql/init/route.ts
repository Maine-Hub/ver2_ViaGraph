...
export async function POST() {
  try {
    ...
    await query(`CREATE TABLE IF NOT EXISTS transfer_legs (id VARCHAR(300) PRIMARY KEY, transfer_id VARCHAR(300) NOT NULL, leg_order INT NOT NULL DEFAULT 0, route_name VARCHAR(255) NOT NULL, vehicle_type VARCHAR(50) DEFAULT 'jeepney', distance DOUBLE NOT NULL, stop_and_transfer TEXT, note TEXT, path_coordinates LONGTEXT, regular_fare DECIMAL(10,2), discounted_fare DECIMAL(10,2), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)`);

    await query(`ALTER TABLE transfer_legs ADD COLUMN IF NOT EXISTS vehicle_type VARCHAR(50) DEFAULT 'jeepney'`).catch(() => { });
    ...
    await query(`CREATE TABLE IF NOT EXISTS fare_rules (
      vehicle_type VARCHAR(50) PRIMARY KEY,
      base_fare DECIMAL(10,2) NOT NULL,
      first_km DECIMAL(10,2) NOT NULL,
      succeeding_km_fare DECIMAL(10,2) NOT NULL,
      discount_percentage DECIMAL(5,2) NOT NULL,
      updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
    )`);

    // Initialize fare rules if not exists
    await query(`INSERT IGNORE INTO fare_rules (vehicle_type, base_fare, first_km, succeeding_km_fare, discount_percentage) VALUES 
      ('jeepney', 13.00, 4.00, 1.80, 0.20),
      ('minibus', 15.00, 4.00, 2.20, 0.20)`);
